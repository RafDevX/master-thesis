...
how data can be used in order to identify potential breaches of both integrity
...
Contributions of relevance include, in addition to the aforementioned
theoretical model, a Rust implementation of the developed analyzer, as well as
other supporting software, a benchmarks corpus comprising $230$ simple Go
modules that showcase a particular flow or aspect relevant for the language, a
Base Security Policy of generic blanket security controls applicable as a
reasonable default before domain-specific configuration, and an evaluation step
where Glowy audited $371$ popular, production-grade, open-source Go modules,
identifying true security vulnerabilities in real-world projects.
...
covering use cases from the most basic to the most complex, including through
Glowy's opt-in orthogonal security axes mechanic for increased expressiveness
when required.
