...
// (adapted to add support for custom ref-numbering in wrapped-enum-numbering)
...
#let wrapped-enum-numbering(ref-numbering: auto, numbering) = {
  let enum-numbering = (..it) => {
    let ref-numbering = if ref-numbering == auto {
      numbering
    } else {
      ref-numbering
    }
...
#let setup-enum-refs(body) = {
  show ref: it => {
    ...
    if el != none and el.func() == metadata and el == enu-label-mark {
      ...
      let loc = el.location()
      let ref-counter = context numbering(
        state("enum-numbering").at(loc),
        ..counter(enum-counter-name).at(loc),
      )
      if is-empty(supp) {
